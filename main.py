...
class HardwareController:
    ...
    try:
        ...
        self.last_watered[pump_id] = int((time.time() + unix_offset) * 1000)
...
class FirebaseClient:
    ...
    """Log error to Firebase with automatic cleanup (stores UTC-based timestamps)"""
    try:
        # Use UNIX epoch correction because MicroPython time.time() is seconds since 2000-01-01
        unix_offset = 946684800  # seconds between 1970 and 2000
        ...
        self.error_count += 1
        # Use UTC-based seconds for key (int), not raw time.time()
        now_unix_s = int(time.time() + unix_offset)
        error_key = f"error_{now_unix_s}_{self.error_count}"

        error_data = {
            "timestamp": int((time.time() + unix_offset) * 1000),  # milliseconds since 1970 UTC
