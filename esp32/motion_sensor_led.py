...
# Energieeffizient mit Deep Sleep und Interrupt
# - Zeigt Wasserstand langsam im Uhrzeigersinn
# - Blinkt 4 Quadranten für Bodenfeuchte (je 3 Sekunden)
# - 3-Minuten-Cooldown nach Anzeige
...
class MotionSensorLED:
    def __init__(self, hardware_controller, watering_system):
        ...
        print("✓ Motion Sensor LED Controller initialized")
    ...
    def can_trigger(self):
        ...
        if self.is_on_cooldown:
            ...
            if elapsed >= self.motion_cooldown:
                ...
                print(f"⏳ Motion sensor cooldown: {remaining:.0f}s remaining")
                return False
        return True
    ...
    def handle_motion_detected(self, sensor_data):
        ...
        if not self.can_trigger():
            print("⚠ Motion detected but cooldown active")
            return False
...
class MotionSensorInterrupt:
    ...
    def __init__(self, motion_pin, callback=None):
        ...
        print("✓ Motion Sensor Interrupt handler initialized")
    ...
    def _isr_handler(self, pin):
        ...
        print("✓ Motion interrupt triggered")
        if self.callback:
            try:
                self.callback()
            except Exception as e:
